...
{
// display log info 
  RCLCPP_INFO_STREAM(this->get_logger(), "NDTScanMatcher is created.");
  ...
  timestamp_ = std::chrono::system_clock::to_time_t(start_time_); 
  ...
  sensor_points_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
    "points_raw", rclcpp::QoS(rclcpp::KeepLast(10)).reliable(),
    std::bind(&NDTScanMatcher::callback_sensor_points, this, std::placeholders::_1),
    sensor_sub_opt);

  // fused_pose_sub = this->create_subscription<geometry_msgs::msg::PoseStamped>(
  //   "fused_pose", 10,
  //   std::bind(&NDTScanMatcher::callback_fused_pose, this, std::placeholders::_1),
  //   initial_ose_sub_opt);
  ...
  odom_pose_pub_ = this->create_publisher<nav_msgs::msg::Odometry>("/ndt_odom", 10); 
  odom_pose_all_pub_ = this->create_publisher<nav_msgs::msg::Odometry>("/ndt_odom_all" , 10);
  ...
  path_publisher_ = this->create_publisher<nav_msgs::msg::Path>("reg_path", 10);
...
{
  RCLCPP_INFO(this->get_logger(), "Callback init guess");
  diagnostics_initial_pose_->clear();
  // print last estimated pose
  if(!is_ndt_fine_){
    last_estimated_pose_ = initial_pose_msg_ptr->pose.pose;
  ...
  sensor_msgs::msg::PointCloud2::ConstSharedPtr sensor_points_msg_in_sensor_frame)
{
  ...
  //   return ;
  // }
  std::cout << "callback_sensor_points" << std::endl;
  ...
  sensor_msgs::msg::PointCloud2::ConstSharedPtr sensor_points_msg_in_sensor_frame)
{
  ...
  // std::cout << "sensor time: " << sensor_points_msg_in_sensor_frame->header.stamp.sec << "." << sensor_points_msg_in_sensor_frame->header.stamp.nanosec << std::endl;
  // std::cout << "delay time: " <<  sensor_points_delay_time_sec << std::endl;
  ...
  pcl::CropBox<PointSource> crop_box_filter;
  crop_box_filter.setInputCloud(sensor_points_in_sensor_frame);
  crop_box_filter.setMin(Eigen::Vector4f(-60, -60, -20, 1));
  crop_box_filter.setMax(Eigen::Vector4f(60, 60, 20, 1));
  pcl::PointCloud<PointSource>::Ptr crop_box_filtered_sensor_points(
    new pcl::PointCloud<PointSource>);
  crop_box_filter.filter(*crop_box_filtered_sensor_points);

  // transform sensor points from sensor-frame to base_link
  try {
    transform_sensor_measurement(
      sensor_frame, param_.frame.base_frame, crop_box_filtered_sensor_points,
  ...
  float leaf_size = 1.0f;  // Default value in meters
  // Optionally override leaf_size with a parameter (e.g., param_.voxel_leaf_size)
  // if (param_.voxel_leaf_size > 0) {
  //   leaf_size = param_.voxel_leaf_size;
  // }
  pcl::VoxelGrid<PointSource> voxel_filter;
  voxel_filter.setInputCloud(sensor_points_in_baselink_frame);
  voxel_filter.setLeafSize(leaf_size, leaf_size, leaf_size);
  pcl::PointCloud<PointSource>::Ptr downsampled_cloud(new pcl::PointCloud<PointSource>);
  voxel_filter.filter(*downsampled_cloud);
  // Replace original cloud with downsampled one.
  sensor_points_in_baselink_frame = downsampled_cloud;
  ...
  // if (last_estimated_pose_) {
  //   // Use the last estimated pose as the initial guess
  //   geometry_msgs::msg::PoseWithCovarianceStamped last_pose_msg;
  //   last_pose_msg.header.stamp = sensor_ros_time;
  //   last_pose_msg.header.frame_id = param_.frame.map_frame;
  //   last_pose_msg.pose.pose = last_estimated_pose_;
  //   interpolation_result_opt = initial_pose_buffer_->interpolate(sensor_ros_time, last_pose_msg);
  // } else {
  //   // Fallback to the initial pose buffer
  //   interpolation_result_opt = initial_pose_buffer_->interpolate(sensor_ros_time);
  // }
  ...
  //              "(2) the t  imestamps of the sensor PCD messages and pose messages are synchronized "
  //              "correctly.";
  //   diagnostics_scan_points_->update_level_and_message(
  //     diagnostic_msgs::msg::DiagnosticStatus::WARN, message.str());
  //     RCLCPP_INFO_STREAM(this->get_logger(), "Couldn't interpolate initial pose .");
  ...
  // interpolation result uses to last estimated pose
  SmartPoseBuffer::InterpolateResult interpolation_result;
  interpolation_result.interpolated_pose.pose.pose = last_estimated_pose_;
  ...
  if (!is_set_map_points) {
    RCLCPP_INFO_STREAM(this->get_logger(), "No map points.");
    std::stringstream message;
  ...
  pclomp::NdtResult ndt_result = ndt_ptr_->getResult();

  // add noise to the pose
  // std::cout << "ndt_result.pose before: \n" << ndt_result.pose << std::endl;
  bool is_converged = 1;
  if (false) {
    ...
    std::mt19937 gen(rd());
    std::normal_distribution<float> noise_dist(0.0,0.5);  // Mean=0, StdDev=noise_level

    // // Add noise to the translation (last column)
    
    for (int i = 0; i < 3; ++i) {
      ndt_result.pose(0, 3) += noise_dist(gen);
    }
    is_converged = 0 ;

    std::cout << "add noise to the pose" << std::endl;
    // std::default_random_engine generator;
    // std::normal_distribution<float> distribution(0.0,1);
    // std::cout << "distribution: " << distribution(generator) << std::endl;
    // ndt_result.pose(0, 3) += distribution(generator);
    // ndt_result.pose(1, 3) += distribution(generator);
    // ndt_result.pose(2, 3) += distribution(generator);
    std::cout << "ndt_result.pose after: \n" << ndt_result.pose << std::endl;
      // point.x += distribution(generator);
      // point.y += distribution(generator);
      // point.z += distribution(generator);
    
  }
  // check is_succeed_scan_matching
  ...
  std::cout << "iteration_num: " << ndt_result.iteration_num<< " " << ndt_ptr_->getMaximumIterations()<< std::endl;
  ...
  if (is_local_optimal_solution_oscillation) {
    ...
    diagnostics_scan_points_->update_level_and_message(
      diagnostic_msgs::msg::DiagnosticStatus::WARN, message.str());
    std::cout << "There is a possibility of oscillation in a local minimum" << std::endl;
  }
  ...
  // RCL_INFO_STREAM(this->get_logger(), "Score: " << str(score) << ", Threshold: " << score_threshold);
  // allow to writ double number not string  in RCL_info_stream  
  std::cout << "Score: " << score << ", Threshold: " << score_threshold << std::endl;
   
  bool is_ok_score = (score > score_threshold);
  if (!is_ok_score) {
    RCLCPP_INFO_STREAM(this->get_logger(), "Below hershold Score ");
    std::stringstream message;
  ...
  // bool is_ok_execution_time = exe_time < param_.validation.critical_upper_bound_exe_time_ms;

  // // check is_converged
  is_converged = (is_ok_iteration_num || is_local_optimal_solution_oscillation) && is_ok_score ;
  // is_converged = 0 ;
  // RCL_INFO_STREAM(this->get_logger(), "is_converged:  " << str(is_converged));
  ...
  std::cout << "Matcing Execution time: " << exe_time << " [ms]" << std::endl;
  ...
  // check is_converged
  // bool is_converged = (is_ok_iteration_num || is_local_optimal_solution_oscillation) ;
  last_estimated_pose_ = result_pose_msg;
  latest_ekf_position_ = result_pose_msg.position;
  // publish
  publish_odom_pose(sensor_ros_time, result_pose_msg, ndt_covariance, is_converged);
  ...
  double timestamp = sensor_points_msg_in_sensor_frame->header.stamp.sec + sensor_points_msg_in_sensor_frame->header.stamp.nanosec * 1e-9;
  is_ndt_fine_ = false;
  if(true){
    std::string reg_log_file = "data/registration_log/reg_log" + std::to_string(timestamp_) + ".txt";
    std::ofstream registration_log;
    registration_log << std::fixed << std::setprecision(9);
    registration_log.open(reg_log_file, std::ios::app);
  ...
  // // save pcd file 
  // if (true) {
  //   std::string pcd_file = "data/pcd_file/" + std::to_string(timestamp) +".pcd";
    // pcl::io::savePCDFileASCII(pcd_file, *sensor_points_in_map_ptr);
  // }
  
  std::cout << "NDT is_converged: " << is_converged << std::endl;
  return is_converged;
}


void NDTScanMatcher::publish_odom_pose(
  const rclcpp::Time & sensor_ros_time, const geometry_msgs::msg::Pose & result_pose_msg,
  const std::array<double, 36> & ndt_covariance, const bool is_converged)
{
 
  nav_msgs::msg::Odometry odom_pose_msg;
  odom_pose_msg.header.stamp = sensor_ros_time;
  odom_pose_msg.header.frame_id = param_.frame.map_frame;
  odom_pose_msg.child_frame_id = param_.frame.ndt_base_frame;
  odom_pose_msg.pose.pose = result_pose_msg;
  odom_pose_msg.pose.covariance = ndt_covariance;

  odom_pose_all_pub_->publish(odom_pose_msg);
...
{ 
  ...
  tf2_broadcaster_.sendTransform(
    autoware::universe_utils::pose2transform(result_pose_stamped_msg, "base_link_ndt"));
}
  ...
  const std::array<double, 36> & ndt_covariance, const bool is_converged)
{
  ...
  path.header.frame_id = param_.frame.map_frame;
  path.pose = result_pose_msg;
  path_.header.stamp = sensor_ros_time;
  path_.header.frame_id = param_.frame.map_frame;

  // string timestamp_ of starttime 
  std::string path_log_file = "data/path_log/ndt_path/ndt_path_log" + std::to_string(timestamp_) + ".txt";
  std::ofstream pose_log;
  pose_log.open(path_log_file, std::ios::app);
  pose_log << " " << result_pose_msg.position.x << " " << result_pose_msg.position.y << " " << result_pose_msg.position.z << " " << result_pose_msg.orientation.x << " " << result_pose_msg.orientation.y << " " << result_pose_msg.orientation.z<< " " <<result_pose_msg.orientation.w << std::endl;
  pose_log.close();
  ...
  if (jjj % 7 == 0) 
  {
    path_.poses.push_back(path);
    path_publisher_->publish(path_);
  
  }
  
  if (is_converged) {
    std::cout << "is_converged: " << "update map" << is_converged << std::endl;
...
{
  // loger
  RCLCPP_INFO(this->get_logger(), "Service ndt_align !");
  const rclcpp::Time ros_time_now = this->now();
  ...
  tier4_localization_msgs::srv::PoseWithCovarianceStamped::Response::SharedPtr res)
{
  ...
  RCLCPP_INFO(this->get_logger(), "Is Map Loaded? %s", is_set_map_points ? "YES" : "NO");
